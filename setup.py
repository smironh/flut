					...
					if int(phone):
						...
						try:
							if int(count):
								...
								iteration = 0
								addparams()
								info = '\nТелефон: {}\nКол-во кругов: {}'.format(phone, count)+'\nСообщение идут.\nнажмите Ctrl+Z если хотите остонавить.'
								clear()
								logo()
								print(info)
								if proxy=="localhost":
							...
							info=Fore.RED+"количестви кругов введено не верно("+Style.RESET_ALL
				except:
					info=Fore.RED+"Номер телефона введён не верно("+Style.RESET_ALL
